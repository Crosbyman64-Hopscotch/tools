...
const getTrait=(t,e,d)=>((t>=2000&&t<=2018)?`object(${getTraitParamType(e,d)})`:((t>=2500&&t<=2507)?"user":"game"))+"."+e.datum.description.replace(/\s([A-Z])/g,(match,group1)=>`_${group1.toLowerCase()}`).replace(/asa%$/,"_%").replace(/([A-Za-z0-9]+)_/g,(match,group1)=>`${group1.toLowerCase()}`)
