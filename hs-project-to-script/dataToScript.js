...
const getTrait=(t,e,d)=>((t>=2000&&t<=2018)?`object(${getTraitParamType(e,d)})`:((t>=2500&&t<=2507)?"user":"game"))+"."+e.datum.description.replace(/([A-Za-z]+)\s([A-Z])/g,(match,group1,group2)=>`${group1.toLowerCase()}_${group2.toLowerCase()}`).replace(/asa%$/,"")
